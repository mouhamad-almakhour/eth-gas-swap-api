...
export class GasPriceService implements OnModuleInit {
  ...
  private async fetchGasPriceFromBlockchain(): Promise<IGasPrice> {
    ...
    const baseFee = block.baseFeePerGas
      ? Number(ethers.utils.formatUnits(block.baseFeePerGas, 'gwei'))
      : 0;
    const priorityFee = feeData.maxPriorityFeePerGas
      ? Number(ethers.utils.formatUnits(feeData.maxPriorityFeePerGas, 'gwei'))
      : 0;
  ...
  async getCachedGasPrice(): Promise<GasPriceResponseDto> {
    ...
    if (!cachedData) {
      throw new Error(
        'Gas price cache not initialized. Please try again in a few seconds.',
      );
    }
