...
describe('UniswapService', () => {
  ...
  it('should return UniswapResponseDto', async () => {
    ...
    const mockPair = {
      getReserves: jest.fn().mockResolvedValue({
        reserve0: ethers.BigNumber.from(1000),
        reserve1: ethers.BigNumber.from(2000),
      }),
    ...
    jest
      ...
      .mockImplementation((address, abi, provider) => {
        ...
        if (address === fromTokenAddress || address === toTokenAddress)
          return mockToken as any;
        return {} as any;
      });

    const payload: GetReturnDto = {
      fromTokenAddress,
      toTokenAddress,
      amountIn,
    };

    const result: UniswapResponseDto = await service.getReturn(payload);
    ...
    expect(mockFactory.getPair).toHaveBeenCalledWith(
      fromTokenAddress,
      toTokenAddress,
    );
  });
  ...
  it('should throw error if pair does not exist', async () => {
    ...
    await expect(service.getReturn(payload)).rejects.toThrow();
  });
});
